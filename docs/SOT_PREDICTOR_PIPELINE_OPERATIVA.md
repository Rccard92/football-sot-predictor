...
2. FE: filtri opportunity → `sortOpportunitiesByEvidenceStrength` dentro fixture → primary = `ops[0]` → sort fixture (per evidence primary o altro criterio scelto).
3. Primary resta evidence-first anche se l’utente ordina per kickoff/V3.1/edge.
4. Filtro «Contesto disponibile» = `context_support.available === true` (non conferma).
5. Soft poll / Sonner invariati; CTA Today invariata.
6. Nessuna write, nessun cart, nessun score aggregato, nessun backend change.
...
2. Aprire Run #3: sezione Acquistabilità mostra V3, Replay ID 1, 36488 / 13534 / 22950 / 4.
3. Menu **Scarica report → Dettaglio Acquistabilità**: ZIP V3 ufficiale.
4. Manifest: `official_purchasability_version=V3`, `legacy_*=false`, `formula_recomputed=false`.
5. **Sintesi per ChatGPT**: sezione `purchasability` solo V3.
6. Nessun fallback V1.1/V2; se replay assente → messaggio + CTA Replay.
...
6. Post-3C.2: usare Dettaglio Acquistabilità / Sintesi ufficiali (V3).
...
4. Report ZIP v4 (+ `signal_opportunities.jsonl`, `signal_models.jsonl` legacy, `goal_intensity.jsonl`, `purchasability_compact.jsonl` / decisions / drift; `purchasability.jsonl` full solo in `full_archive`)
...
4. Lettura UI: `/api/cecchino-lab/overview|analytics/overview|datasets|matches|data-quality/issues`.
5. Export qualità: `GET /api/cecchino-lab/data-quality/issues/export` (CSV/JSON, scope filtered|all, no pagination).
6. Replace controllato: `POST /api/admin/cecchino-lab/datasets/{id}/replace` + `REPLACE_CECCHINO_LAB_DATASET`.
7. Import multiplo: `POST /api/admin/cecchino-lab/imports/batch/preview` (stagione unica, mapping Div automatico, no write) → import sequenziale sull’endpoint singolo; nessun auto-replace; fallimento di un file non annulla i completati.
8. **Nessun** collegamento a `cecchino_today_fixtures`, scan, formule, Balance/GI/Segnali.
