...
Manifest v2, snapshot Balance `balance_v5_monitoring`, ZIP Goal/Segnali con file reali, health Acquistabilità diagnostico, `export-status` + UI completezza. Coorti prospettiche ≠ legacy. Formule invariate. Next: Fase 2/3 empirica Balance su coorte prospettico.
...
`fixed_discount_v3` (`cecchino_purchasability_v3_candidate_1`) → snapshot `purchasability_preview_v3`. Gate valore positivo → score null se fallisce. Value solo da Edge (scala fissa /50); qualità come sconto da penalità reali; formula `value×quality/100`. Nessun profilo storico, nessun percentile. **STEP 2.2 UI:** nel Pannello KPI V3 è l’unica colonna **Acquistabilità** (niente badge candidato); V2 rimossa solo dalla UI KPI (backend/API/audit/export preservati); popup semplificato (risultato → gate → valore → penalità applicate → famiglia → finale) con dettagli tecnici richiudibili; linked context solo nei dettagli; formula invariata; replay storico ancora non eseguito (STEP 3). Vedi `SOT_PREDICTOR_PURCHASABILITY_RESEARCH.md`.

## Acquistabilità v2 — Indice decisionale parallelo (2026-07-27)

`decision_quality_v2` (`cecchino_purchasability_v2_candidate_1`) è un indice **parallelo** alla v1.1. Non sostituisce `balanced_geometric_v1_1` né la validation pipeline. Snapshot separato: `cecchino_output_json.purchasability_preview_v2`. Normalizzazione storica congelata (`cecchino_purchasability_v2_norm_profile_2026_07_26_v2`, cutoff `scan_date <= 2026-07-26`): solo fixture `eligible` con timestamp snapshot verificato e `snapshot_at < kickoff` (`updated_at` non è prova pre-match). UI KPI (post STEP 2.2): colonna unica Acquistabilità = V3; V2 non più in colonna KPI ma ancora calcolata e persistita. Backfill manuale: default dry-run non scrivente; `--apply` + token solo dopo guardie. Formula v2 invariata. Cecchino Lab non modificato. Vedi `SOT_PREDICTOR_PURCHASABILITY_RESEARCH.md`.
...
- Monitoraggio Segnali — pagina aggregata `/monitoraggio-segnali` (heatmap UNDER/OVER 2.5, KPI con media segnali/partita Fase 35, Quota media prese / Quota Void / Rendimento prese Fase 42, **Confronto modelli pesi A–F Fase 43**, **formule segnali V3 + coorti Monitoraggio V1/V2** (`monitoring_version`: V1 = SI value-gated anche 1 conferma; V2 default = consenso ≥2 stesso segno + HOME/AWAY single-formula), export CSV, backfill/revaluate/backtest V3-only, pulsante «Ricalcola modelli A–F», pulsante «Ricalcola filtro valore» (value gate quota book ≥ Cecchino), diagnostics versionate v1/v2/v3, legenda da `GET …/signals/formula-registry` Fase 39/45)
