...
## Availability / Indisponibili (step 8A) — disattivato

La componente indisponibili/infortuni è stata **rimossa** perché i dati da API-Sports (`injuries`, `sidelined`) e valutazioni Sportmonks si sono dimostrati non affidabili. Il modello SOT Prediction **non utilizza** attualmente assenze o infortuni nel calcolo (`availability_adjustment`: `status: disabled`, penalità 0).

- **Tabelle DB:** `player_availability` e `player_availability_events` restano in schema (dati storici ignorati; nessuna migration drop).
