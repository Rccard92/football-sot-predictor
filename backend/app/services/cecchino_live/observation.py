...
def observation_dashboard(db: Session, *, date_from: date | None = None, date_to: date | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "models_overview": _models_overview(db, rows),
...
def _models_overview(db: Session, rows: list[CecchinoLivePrediction]) -> dict[str, Any]:
    from app.services.cecchino_live.observation_models import models_overview

    return models_overview(db, rows)
