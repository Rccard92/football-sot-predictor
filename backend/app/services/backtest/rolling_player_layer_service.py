...
class RollingPlayerLayerService:
    def build_team_player_layer(
        ...
        *,
        ...
        if fixture is None:
            return self._neutral_fallback(["fixture_not_found"])

        home_missing, away_missing = load_sportapi_missing_by_side(db, int(fixture_id))
        missing_rows: list[FixtureMissingPlayer] = home_missing if side == "home" else away_missing

        coverage, starters_raw, bench_raw, _unavailable_raw = resolve_side_lineup(
            db,
            fixture=fixture,
            team_id=int(team_id),
            side=side,
        ...
        if not coverage.has_official_xi or not starters_raw:
