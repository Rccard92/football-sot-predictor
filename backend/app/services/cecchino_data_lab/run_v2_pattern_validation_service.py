...
def start_validation(
    ...
    *,
    run_v2_run_id: int,
    final_lockbox_test: bool = False,
    insight_run_id: int | None = None,
    spawn: bool = True,
) -> dict[str, Any]:
    """final_lockbox_test: unico uso ammesso della stagione sotto chiave, il test
    finale di confronto con i pattern gia' congelati. insight_run_id: analisi di
    scoperta esplicita (la RUN V2.5 ha la sua); spawn=False esegue nello stesso thread."""
    q = select(CecchinoRunV2PatternInsightRun).where(CecchinoRunV2PatternInsightRun.status == STATUS_COMPLETED)
    if insight_run_id is not None:
        q = q.where(CecchinoRunV2PatternInsightRun.id == int(insight_run_id))
    insight = db.scalars(q.order_by(CecchinoRunV2PatternInsightRun.completed_at.desc())).first()
    if not insight:
    ...
    if not target or target.status not in ("completed", "completed_with_warnings"):
        raise CecchinoLabImportError(
    ...
    if discovery.run_version != target.run_version:
        raise CecchinoLabImportError(
            "version_mismatch", "Scoperta e verifica devono venire dallo stesso motore", status_code=400
        )
    ...
    if spawn:
        ...
        _execute_validation(int(run.id))
        db.refresh(run)
