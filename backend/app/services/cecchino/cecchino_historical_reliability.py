"""Affidabilità storica v1.1 — copertura operativa Pannello KPI.

Versione: cecchino_historical_reliability_v1_1
metric_kind: historical_reliability

Misura il comportamento storico dello stesso mercato e della stessa fascia Rating.
Non è l'Acquistabilità (modulo separato, non calcolato in Fase 1/5).
Formula score invariata rispetto a cecchino_purchasability_empirical_rating_v1_1.
Current rows da kpi_panel_json.
"""
...
HISTORICAL_RELIABILITY_VERSION = "cecchino_historical_reliability_v1_1"
LEGACY_EMPIRICAL_VERSION = "cecchino_purchasability_empirical_rating_v1_1"
METRIC_KIND = "historical_reliability"
...
def build_historical_reliability_index(
...
def build_historical_reliability_global_index(
    rows: list[dict[str, Any]],
...
def calculate_historical_reliability_cohort_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
...
def _explanation(
    ...
    *,
    ...
    meta = cohort_meta or {}
    if status == "rating_below_scope":
        return "L’Affidabilità storica viene calcolata per Rating almeno pari a 50."
    if status == "unsupported_market":
    ...
    return (
        "Affidabilità storica basata sul comportamento dello stesso mercato "
        "nella fascia Rating osservata."
    )
...
def calculate_historical_reliability(
    ...
    *,
    ...
    out = {
        ...
        "version": HISTORICAL_RELIABILITY_VERSION,
        "legacy_version": LEGACY_EMPIRICAL_VERSION,
        "status": status,
        "score": score,
        "class": klass,
...
def build_historical_reliability_for_panel(
    db: Session,
    *,
    date_from: date,
    date_to: date,
