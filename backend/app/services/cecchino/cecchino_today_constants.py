"""Costanti Cecchino Today discovery."""

from __future__ import annotations

CECCHINO_TODAY_VERSION = "cecchino_today_v0_13_api_gates"
DEFAULT_TODAY_TIMEZONE = "Europe/Rome"
DEFAULT_RETENTION_DAYS = 7
CECCHINO_TODAY_TIMELINE_WINDOW_DAYS = 30
CECCHINO_CLEANUP_CONFIRM_TOKEN = "DELETE_CECCHINO_HISTORY"
PROVIDER_API_FOOTBALL = "api_football"

MIN_HOME_CONTEXT = 3
MIN_AWAY_CONTEXT = 3
MIN_HOME_TOTAL = 6
MIN_AWAY_TOTAL = 6
MIN_RECENT_CONTEXT_5 = 3
MIN_RECENT_TOTAL_6 = 5

# Marker femminili inequivocabili (token/phrase). Nessuna sola lettera "w".
WOMEN_KEYWORDS = frozenset(
    {
        "women",
        "woman",
        "female",
        "femminile",
        "femminili",
        "féminin",
        "feminin",
        "femenina",
        "femenino",
        "femenil",
        "feminina",
        "feminino",
        "frauen",
        "ladies",
        "mujeres",
        "w-league",
        "w league",
    },
)

CUP_KEYWORDS = frozenset(
    {
        "cup",
        "coppa",
        "copa",
        "coupe",
        "pokal",
        "trophy",
        "super cup",
        "supercup",
        "shield",
        "champions league",
        "europa league",
        "conference league",
        "libertadores",
        "sudamericana",
        "fa cup",
        "league cup",
        "carabao",
        "copa del rey",
        "dfb pokal",
        "coppa italia",
    },
)

FRIENDLY_KEYWORDS = frozenset(
    {
        "friendly",
        "friendlies",
        "club friendly",
        "international friendly",
        "amichevole",
        "amichevoli",
        "amistoso",
        "amistosos",
        "amigable",
    },
)

YOUTH_KEYWORDS = frozenset(
    {
        "u17",
        "u18",
        "u19",
        "u20",
        "u21",
        "u22",
        "u23",
        "under 17",
        "under 18",
        "under 19",
        "under 20",
        "under 21",
        "under 22",
        "under 23",
        "sub-17",
        "sub-18",
        "sub-19",
        "sub-20",
        "sub-21",
        "sub-22",
        "sub-23",
        "youth",
        "primavera",
        "reserve",
        "reserves",
    },
)
