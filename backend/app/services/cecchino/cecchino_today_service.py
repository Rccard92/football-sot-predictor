        ...
        for item in batch:
            ...
            try:
                ...
                try:
                    ...
                    pass
                existing_prev_v35 = None
                if existing_row is not None and isinstance(
                    existing_row.cecchino_output_json, dict
                ):
                    existing_prev_v35 = existing_row.cecchino_output_json.get(
                        "purchasability_preview_v35"
                    )
                try:
                    attach_purchasability_preview_v35_to_output(
                        cecchino_output=cecchino_output,
                        ...
                        existing_preview_v35=existing_prev_v35
                        if isinstance(existing_prev_v35, dict)
                        else None,
                    )
                except Exception as exc:
                    logger.warning(
...
def get_today_fixture_detail(db: Session, today_fixture_id: int) -> dict[str, Any] | None:
    ...
    output = technical["cecchino_output"]
    kpi_panel = technical["kpi_panel"]
    expected_goal_engine_diagnostics = technical["expected_goal_engine_diagnostics"]
    fixture_identity_consistency = technical["fixture_identity_consistency"]
    balance_v5_snapshot_meta = technical["balance_v5_snapshot_meta"]
    balance_v5 = technical["balance_v5"]
    goal_intensity_v5 = technical["goal_intensity_v5"]
    goal_intensity_v5_preview = technical["goal_intensity_v5_preview"]
    signals_matrix = technical["signals_matrix"]
    signal_contract = technical["signal_contract"]
