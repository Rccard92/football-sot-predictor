"""Export audit Acquistabilità V3.5 Structural V2 — read-only, persisted snapshot only."""
...
def _assert_no_post_match_leakage_v35_v2(payload: dict[str, Any]) -> None:
...
def _fixture_block(row: CecchinoTodayFixture) -> dict[str, Any]:
    return {
        ...
        "date": row.scan_date.isoformat() if row.scan_date else None,
...
def build_purchasability_v35_v2_audit_export(
    ...
    payload = make_json_safe(
        {
            "contract_version": PURCHASABILITY_V35_V2_AUDIT_EXPORT_CONTRACT_VERSION,
    ...
    _assert_no_post_match_leakage_v35_v2(payload)
    return payload


class V35V2SnapshotUnavailableError(Exception):
    code = "v35_v2_snapshot_unavailable"


class V35V2SnapshotInvalidError(Exception):
    code = "v35_v2_snapshot_invalid"


def get_purchasability_v35_v2_audit_export(
    db: Session,
    ...
    if not isinstance(snapshot, dict):
        raise V35V2SnapshotUnavailableError()

    check = validate_purchasability_preview_v35_v2_snapshot(snapshot)
    if not check.get("ok"):
        raise V35V2SnapshotInvalidError()

    export = build_purchasability_v35_v2_audit_export(row, snapshot)
    ts = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    filename = f"purchasability-v35-v2-audit-{int(row.provider_fixture_id)}-{ts}.json"
