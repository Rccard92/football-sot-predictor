...
class PlayerSotProfile(Base, TimestampMixin):
    ...
    assists_total: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    key_passes_total: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    key_passes_per90: Mapped[float | None] = mapped_column(Float, nullable=True)
    avg_rating: Mapped[float | None] = mapped_column(Float, nullable=True)
    recent_minutes_last5: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    recent_rating_last5: Mapped[float | None] = mapped_column(Float, nullable=True)
    team_total_shots_share_pct: Mapped[float | None] = mapped_column(Float, nullable=True)
