...
def test_missing_pinnacle():
    ...
    ok, _, reason, blocking = verify_complete_1x2_odds(odds)
    assert not ok
    assert reason == "missing_bookmaker"
    assert any("missing_bookmaker:Pinnacle" in b for b in blocking)
...
def test_incomplete_1x2_market():
    ...
    assert any("missing_selection:" in b for b in blocking)
