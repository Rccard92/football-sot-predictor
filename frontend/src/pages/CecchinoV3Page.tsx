...
export function CecchinoV3Page() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await getLatestV3Runs()
  ...
  const evaluation = completed?.summary?.evaluation
  const config = (completed ?? latest)?.config
    ...
    <PatternInsightsShell>
      <header className="mb-5">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="pi-chip">Fase 1 · Specialista Forza</span>
          {config ? (
            <>
        ...
        <p className="mt-2 max-w-4xl text-xs leading-relaxed" style={{ color: 'var(--pi-muted)' }}>
          ...
          dei giorni precedenti. Da un&apos;unica distribuzione dei risultati escono tutti i 17 mercati.
        </p>
      </header>
