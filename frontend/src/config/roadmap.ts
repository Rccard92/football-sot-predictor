...
export const ROADMAP_STEPS: RoadmapStep[] = [
  {
    ...
    dependsOn: [],
    tasks: [
      { text: 'Salvare il risultato di ogni pattern V3 sul 2025/26 (per la V2 i dati ci sono gia)', status: 'da_fare' },
      { text: 'Ricerca pattern V3 sui mercati senza quota (tiri, corner, cartellini) con lo stesso metodo della V2', status: 'da_fare' },
      { text: 'Pagina con 3 riquadri (V2 · V2.5 in costruzione · V3), filtri e tabella dei pattern 4/4', status: 'da_fare' },
      { text: 'Mercati con quota (profitto, ROI, quota media) e senza quota (frequenza e scostamento dalla media)', status: 'da_fare' },
      { text: 'Confronto con quanti pattern vincenti darebbe il caso', status: 'da_fare' },
      { text: 'Dettaglio al clic: stagione per stagione ed elenco partite', status: 'da_fare' },
      { text: 'Ogni pattern salvato con la versione del motore che lo ha generato', status: 'da_fare' },
    ],
  ...
  },
  {
    ...
    dependsOn: [],
    tasks: [
      { text: 'Cosa fornisce oggi la fonte di Cecchino Today: gol, tiri, tiri in porta, corner, cartellini', status: 'da_fare' },
      { text: 'Allineamento nomi squadra e calendario con lo storico', status: 'da_fare' },
      { text: 'Tabella campionato → dati disponibili → motori che possono girare', status: 'da_fare' },
    ],
