...
const METRICS: Array<{ key: keyof KpiSignalsBucket; label: string; format?: (v: unknown) => string }> = [
  { key: 'activations', label: 'Segnali KPI' },
  { key: 'settled', label: 'Valutati' },
  { key: 'won', label: 'Vinti' },
  { key: 'lost', label: 'Persi' },
  { key: 'win_rate', label: 'Win rate', format: (v) => (v == null ? '—' : `${Number(v).toFixed(1)}%`) },
  ...
  { key: 'profit_units', label: 'Profitto unità', format: (v) => (v == null ? '—' : Number(v).toFixed(2)) },
  { key: 'roi_pct', label: 'ROI %', format: (v) => (v == null ? '—' : `${Number(v).toFixed(2)}%`) },
]
    ...
    <section className="rounded-2xl border border-slate-200/80 bg-white/80 p-4 shadow-sm">
      ...
      >
        {METRICS.map(({ key, label, format }) => {
          ...
          return (
            <div key={key} className="rounded-xl border border-slate-100 bg-gradient-to-br from-white to-slate-50/80 px-3 py-3 shadow-sm">
              <p className="text-[10px] font-medium uppercase tracking-wide text-slate-500">{label}</p>
              <p className="mt-1 text-xl font-semibold tabular-nums text-slate-900">{display}</p>
            </div>
          )
        })}
      </motion.div>
