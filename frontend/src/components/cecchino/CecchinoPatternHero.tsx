...
import { breakEvenQuota, groupPatternSignals, referencePattern, type PatternGroupView, type PatternRelation } from './cecchinoPatternUtils'
...
export function CecchinoPatternHero({
  ...
  const { groups, bookGroups } = useMemo(() => {
    const market = (patterns?.active ?? []).filter((x) => x.target_type === 'market')
    return {
      groups: groupPatternSignals(market.filter((x) => !x.uses_book)),
      bookGroups: groupPatternSignals(market.filter((x) => x.uses_book)),
    }
  }, [patterns])
  const [openKey, setOpenKey] = useState<string | null>(null)
  const results = p?.result?.markets ?? {}

  const renderGroup = (g: PatternGroupView) => {
      ...
      <div className="bg-[#163352] p-3">
        {patterns?.status !== 'ok' ? (
          ...
          <p className="px-1 py-2 text-sm text-slate-300">
            Nessun pattern con quota acceso sui soli moduli: per la {model} questa partita non ha una giocata indicata.
          </p>
          ) : (
          <div className="grid gap-2 lg:grid-cols-2">{groups.map(renderGroup)}</div>
          )}
          {bookGroups.length > 0 && (
            <div className="mt-3 border-t border-slate-500/40 pt-3">
              <p className="mb-2 px-1 text-xs font-semibold uppercase tracking-wide text-amber-200">
                Pattern che usano la quota come condizione · da leggere a parte
              </p>
              <div className="grid gap-2 lg:grid-cols-2">{bookGroups.map((g) => renderGroup({ ...g, key: `book|${g.key}` }))}</div>
            </div>
          )}
          </>
        )}
