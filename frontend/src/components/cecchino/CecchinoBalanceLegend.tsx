import {
  ...
  DELTA_FORCE_LEGEND_NOTE,
  DELTA_FORCE_LEGEND_ROWS,
  type BalanceLegendSeverity,
  type BalanceOperationalLegendRow,
  type DeltaForceLegendRow,
...
function DeltaForceLegendTable({ rows }: { rows: DeltaForceLegendRow[] }) {
  return (
    <div className="hidden overflow-x-auto md:block">
      <table className="min-w-full text-xs">
        <thead className="bg-slate-100 text-slate-600">
          <tr>
            <th className="px-2 py-2 text-left font-medium">Delta Forza</th>
            <th className="px-2 py-2 text-left font-medium">Classe</th>
            <th className="px-2 py-2 text-left font-medium">Lettura</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {rows.map((row, idx) => (
            <tr key={idx} className={rowBg(row.severity)}>
              <td className="px-2 py-2 whitespace-nowrap text-slate-800">{row.deltaRange}</td>
              <td className="px-2 py-2 whitespace-nowrap text-slate-800">{row.classLabel}</td>
              <td className="px-2 py-2 text-slate-800">{row.reading}</td>
            </tr>
          ))}
        </tbody>
...
function DeltaForceLegendCards({ rows }: { rows: DeltaForceLegendRow[] }) {
  return (
    <div className="space-y-2 md:hidden">
      {rows.map((row, idx) => (
        <article
        ...
        >
          <p className="font-medium text-slate-800">
            {row.deltaRange} — {row.classLabel}
          </p>
          <p className="mt-1 text-slate-800">{row.reading}</p>
        </article>
      ))}
    </div>
  )
    ...
    <details className="rounded-lg border border-slate-200 bg-white text-sm">
      ...
      <div className="border-t border-slate-200 px-4 py-3 space-y-3">
        ...
        <div className="space-y-2 border-t border-slate-100 pt-3">
          ...
          <DeltaForceLegendTable rows={DELTA_FORCE_LEGEND_ROWS} />
          <DeltaForceLegendCards rows={DELTA_FORCE_LEGEND_ROWS} />
          <p className="text-xs text-slate-600">{DELTA_FORCE_LEGEND_NOTE}</p>
        </div>
