...
import { familyLabel, leagueName, v4Label, v4SectionTitle } from './constants'
...
function roiBarOption(groups: ChartGroup[]): Record<string, unknown> {
  ...
  return {
    grid: { left: 64, right: 16, top: 16, bottom: 72 },
    tooltip: {
      trigger: 'axis',
    ...
    xAxis: {
      type: 'category',
      data: labels,
      axisLabel: { fontSize: 14, interval: 0, rotate: labels.length > 6 ? 30 : 0 },
    },
    yAxis: {
      type: 'value',
      axisLabel: { fontSize: 14, formatter: (v: number) => `${String(v).replace('.', ',')}%` },
...
export function V4MeasureView({ api }: Props) {
  ...
  return (
    <div className="space-y-6" data-testid="v4-measure">
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Kpi
          label="CLV medio"
          value={fmtSignedPct(data.totals.clv, 1)}
          hint="Quota presa contro quota di chiusura"
        />
      </div>

      <div className="grid gap-4 xl:grid-cols-2">
        <section className={`${todayCard} ${todayCardPadding} space-y-2`}>
          <h3 className={v4SectionTitle}>ROI per campionato</h3>
          {leagueOption && data.by_league.length > 0 ? (
            <LabEChartsCore echarts={echarts} option={leagueOption} style={{ height: 300 }} notMerge lazyUpdate />
          ) : (
            <p className="text-base text-slate-500">Nessun campionato con giocate regolate.</p>
          )}
        </section>
        <section className={`${todayCard} ${todayCardPadding} space-y-2`}>
          <h3 className={v4SectionTitle}>ROI per famiglia di mercato</h3>
          {marketOption && data.by_market.length > 0 ? (
            <LabEChartsCore echarts={echarts} option={marketOption} style={{ height: 300 }} notMerge lazyUpdate />
          ) : (
            <p className="text-base text-slate-500">Nessuna famiglia con giocate regolate.</p>
          )}
        </section>
      </div>

      <section className={`${todayCard} ${todayCardPadding} space-y-3`}>
        <h3 className={v4SectionTitle}>Allarmi CUSUM</h3>
        {data.alerts.length === 0 ? (
          <p className="text-base text-slate-500">Nessun campionato o mercato in decadimento.</p>
        ) : (
          <ul className="space-y-2">
            {data.alerts.map((a) => (
              <li
                key={`${a.scope}-${a.key}`}
                className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-base text-amber-900"
              ...
              >
                ...
                : {a.sentence}
                <span className="block text-base text-amber-800">
      ...
      {data.postmortem && data.postmortem.length > 0 ? (
        <section className={`${todayCard} ${todayCardPadding} space-y-3`}>
          <h3 className={v4SectionTitle}>Post-mortem della settimana</h3>
