...
function Bar({ label, value, max, mean, tone }: { label: string; value: number; max: number; mean: number; tone: string }) {
  ...
  return (
    <div className="grid grid-cols-[minmax(100px,1fr)_2fr_auto] items-center gap-2">
      <span className="truncate text-xs text-slate-600">{label}</span>
      <div className="relative h-2 overflow-hidden rounded-full bg-slate-100" aria-hidden>
        <div className={`h-full rounded-full ${tone}`} style={{ width: `${pct}%` }} />
        <div className="absolute inset-y-0 w-0.5 bg-slate-700" style={{ left: `${meanPct}%` }} title="Media divisione" />
      </div>
      <span className={`${v4Mono} w-10 text-right text-xs font-semibold text-slate-900`}>{fmtDecimal(value, 1)}</span>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
      {rows.map((r) => {
        ...
        return (
          <div key={r.stat} className="space-y-1.5" data-testid={`v4-how-${r.stat}`}>
            <p className="text-sm font-medium text-slate-900">
              {capitalize(r.label)}
              <span className="ml-1 text-xs font-normal text-slate-500">
                · media divisione {fmtDecimal(r.division_mean, 1)}
      ...
      <p className="text-xs text-slate-500">La linea scura su ogni barra è la media della divisione.</p>
    </div>
  )
}
