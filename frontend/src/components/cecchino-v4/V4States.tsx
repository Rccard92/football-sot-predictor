  ...
  return (
    <div className="space-y-3" role="status" aria-live="polite" data-testid="v4-loading">
      ...
      {Array.from({ length: rows }, (_, i) => (
        <div key={i} className={`${todaySkeleton} h-14 w-full`} />
      ))}
    </div>
  )
}
...
export function V4Error({ message, onRetry }: { message: string; onRetry?: () => void }) {
  return (
    <div
      className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-2 text-sm text-red-800"
      role="alert"
...
export function V4Empty({ title, hint }: { title: string; hint?: string }) {
  return (
    <div
      className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-8 text-center"
