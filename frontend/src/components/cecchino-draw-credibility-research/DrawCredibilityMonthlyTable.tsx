import type { DrawCredibilityMonthRow } from '../../lib/cecchinoDrawCredibilityResearchApi'

type Props = {
  rows: DrawCredibilityMonthRow[]
}

export function DrawCredibilityMonthlyTable({ rows }: Props) {
  if (rows.length === 0) {
    return (
      <section className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm">
        <h2 className="text-sm font-semibold text-slate-900">Distribuzione mensile</h2>
        <p className="mt-2 text-sm text-slate-500">Nessun dato mensile.</p>
      </section>
    )
  }
