import {
  ...
  type OverallXiBreakdown,
} from '../../utils/lineupOverallXi'
...
const CONFIDENCE_BADGE: Record<string, string> = {
  ...
  bassa: 'border-orange-200 bg-orange-50 text-orange-950',
  molto_bassa: 'border-rose-200 bg-rose-50 text-rose-900',
}
...
export function SportApiOverallXiCard({ breakdown }: { breakdown: OverallXiBreakdown }) {
  ...
  const confLabel = confidenceLabelDisplay(breakdown.confidence_level)
    ...
    <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-3">
      ...
      {breakdown.confidence_score != null ? (
        <p className="mt-1 text-[11px] text-slate-700">
          Confidence dato:{' '}
          <span className="font-semibold tabular-nums text-slate-900">{breakdown.confidence_score}/100</span>
          <span className="text-slate-500"> — </span>
          <span className={`inline-flex rounded-full border px-2 py-0.5 text-[10px] font-medium ${confClass}`}>
            {confLabel}
          </span>
        </p>
      ) : null}
      ...
      <div className="mt-3 space-y-1 border-t border-indigo-100/80 pt-2">
        <ScoreRow
          label="Forza squadra base"
          value={breakdown.team_base_strength_score}
          partial={breakdown.partial}
        />
        <ScoreRow label="Attacco SOT titolari" value={breakdown.attacking_sot_score} partial={breakdown.partial} />
        <ScoreRow
          label="Presenza top player"
          value={breakdown.top_players_presence_score}
          partial={breakdown.partial}
        />
        <ScoreRow
